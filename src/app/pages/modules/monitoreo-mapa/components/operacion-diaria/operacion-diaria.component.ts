...
@Component({
  ...
  styleUrl: './operacion-diaria.component.css'
})
export class OperacionDiariaComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  // ── Operación diaria (módulo en continuo, clonado de simulación) ──
  /** true cuando este componente opera como "Operación diaria" (tiempo real, sin fin). */
  modoOperacion = false;
  ...
  // ── Operación diaria: estadía en destino (parámetro configurable) ──
  /** Minutos que una maleta permanece en el almacén de destino tras llegar,
   *  antes de desaparecer (entregada). Configurable en pantalla. */
  ...
  ngOnDestroy(): void {
    ...
    if (!this.modoOperacion && (this.estado === 'listo' || this.estado === 'streaming')) {
      this.sesion.guardar({
        vuelos: this.vuelos,
...
//
  ...
  cargarSimulacionesActivas(): void {
    // Operación diaria: no aplica la lista de simulaciones compartidas.
    if (this.modoOperacion) { this.simulacionesActivas = []; return; }
  ...
  // ── OPERACIÓN DIARIA (en vivo, continua) ───────────────────
  iniciarOperacionDiaria(): void {
    this.detener();
    this.cerrarWs();
    this.limpiarEstadoSim();
    this.modoOperacion = true;
    this.esEspectador = true;   // sin controles de reproducción del usuario
    this.esperandoSync = false;
    this.estado = 'cargando';
    this.mostrarConfig = false;
  ...
  private conectarOperacionDiaria(): void {
    this.cerrarWs();
    const wsUrl = this.simulacionService.getWsUrl().replace('/ws/simulacion', '/ws/operacion-diaria');
    this.ngZone.runOutsideAngular(() => {
      this.ws = new WebSocket(wsUrl);

      this.ws.onopen = () => {
        // La operación diaria NO envía START: el backend emite INIT al conectar.
        this.ngZone.run(() => {
          this.estado = 'streaming';
          this.mensajeProgreso = 'Operación diaria en vivo';
          this.cdr.detectChanges();
        });
      };
      ...
      this.ws.onmessage = (event: MessageEvent) => {
        this.ngZone.run(() => {
          try { this.manejarMensajeWs(JSON.parse(event.data)); }
          catch (e) { console.error('Error parsing WS operacion diaria', e); }
        });
      };

      this.ws.onerror = () => {
        this.ngZone.run(() => { this.mensajeProgreso = ''; this.cdr.detectChanges(); });
      };

      this.ws.onclose = () => {
        this.ngZone.run(() => {
          // Reconexión automática: la operación diaria es continua.
          if (this.modoOperacion) {
            this.mensajeProgreso = 'Reconectando con la operación diaria...';
            this.cdr.detectChanges();
            setTimeout(() => { if (this.modoOperacion) this.conectarOperacionDiaria(); }, 3000);
          }
        });
      };
    });
  ...
  private onWsInit(msg: any): void {
    const tiempoInicioMs = (msg.tiempoRealMs ?? msg.tiempoSimulacionMs) as number;
    this.tiempoInicioMs = tiempoInicioMs;
    this.tiempoActualMs = this.modoOperacion ? Date.now() : tiempoInicioMs;
    // Operación diaria: en continuo, sin corte. Simulación: "foto" de 5 días.
    this.tiempoFinMs    = this.modoOperacion
      ? tiempoInicioMs + 3650 * 24 * this.HORA_MS
      : tiempoInicioMs + this.duracionFotoMs;
    ...
    // Operación diaria: reloj en tiempo real (continuo). Simulación: reproducción a K.
    if (!this.intervalId) {
      if (this.modoOperacion) { this.iniciarRelojOperacion(); }
      else if (!this.esperandoSync) { this.iniciarAutoPlayStreaming(); }
    }

    this.mensajeProgreso = this.modoOperacion ? 'Operación diaria en vivo' : 'Simulación iniciada · primer ciclo ALNS en 10 seg...';
    this.cdr.detectChanges();
  }

  /** Reloj en TIEMPO REAL para la operación diaria (sin aceleración, sin corte). */
  private iniciarRelojOperacion(): void {
    if (this.intervalId) return;
    this.reproduciendo = true;
    this.ngZone.runOutsideAngular(() => {
      this.intervalId = setInterval(() => {
        this.tiempoActualMs = Date.now();
        this.actualizarPosiciones();
  ...
  private onWsUpdate(msg: any): void {
    ...
    const finVentana = (msg.tiempoRealMs ?? msg.tiempoSimulacionMs) as number;
    ...
    // Operación diaria: continua (no se recalcula un corte). Simulación: foto de 5 días.
    if (!this.modoOperacion) {
  ...
  private recomputarDisponibilidadAlmacen(): void {
    ...
    tramosPorEnvio.forEach((tramos, idEnvio) => {
      ...
      // Arribo final: último tramo → almacén destino (para la estadía en destino)
      const ultimo = tramos[tramos.length - 1].vuelo;
      const env = ultimo.envios.find(e => e.idEnvio === idEnvio);
      this.arriboDestino.set(idEnvio, {
        destino: ultimo.destino,
        llegadaMs: ultimo.horaLlegada.getTime(),
        cantidad: env?.cantidad ?? 0
      });
    });
  ...
  // ── OPERACIÓN DIARIA: hora por zona horaria del aeropuerto ──
  /** Aeropuerto de referencia para la "hora local": el seleccionado o Lima por defecto. */
  get refAeropuertoCodigo(): string {
    if (this.aeropuertoFiltroMapa && this.aeropuertoMap.has(this.aeropuertoFiltroMapa)) return this.aeropuertoFiltroMapa;
    if (this.aeropuertoMap.has('SPIM')) return 'SPIM'; // Lima (base del operador)
    return this.aeropuertos[0]?.codigoOaci ?? 'UTC';
  }

  /** Hora local (tiempo real) del aeropuerto de referencia, según su huso horario. */
  get horaAeropuertoDisplay(): string {
    return this.fmtLocalHora(Date.now(), this.gmtDe(this.refAeropuertoCodigo));
  }

  /** Hora real (local del navegador), solo HH:mm dd/MM. */
  get horaRealDisplay(): string {
    const now = new Date();
    const d = now.getDate().toString().padStart(2, '0');
    const m = (now.getMonth() + 1).toString().padStart(2, '0');
    const h = now.getHours().toString().padStart(2, '0');
